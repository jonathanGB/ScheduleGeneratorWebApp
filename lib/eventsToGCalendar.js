...
var _gCalendar = {
	...
	insertEvent: function (auth, event, callback, backoff = 1000) {
		calendar.events.insert({
			...
			if (err && backoff <= 16000) { // exponential-backoff, unless backoff has reached 16s
				setTimeout(() => {
					this.insertEvent(auth, event, callback, backoff * 2);
				}, backoff);
			} else if (err) {
				console.error('err', err);
				callback(false);
			} else {
				const insertedEventId = response.id;
				console.log(`backoff: ${backoff == 1000 ? 0 : backoff / 2}ms`); // show the backoff applied
				console.log(insertedEventId);
				console.log()
				callback(true, insertedEventId);
			}
		});
	},

	deleteEvent(auth, eventId, callback, backoff = 1000) {
		calendar.events.delete({
			...
			if (err && backoff <= 16000) { // exponential-backoff, unless backoff has reached 16s
				setTimeout(() => {
					this.deleteEvent(auth, eventId, callback, backoff * 2);
				}, backoff);
			} else if (err) {
				console.error('err', err);
				callback(err);
